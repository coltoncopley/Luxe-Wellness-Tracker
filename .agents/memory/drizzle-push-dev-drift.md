...
- Phantom prompt (2026-07): `drizzle-kit push` insists on adding `reward_events_user_dedupe_unique` even though that constraint already exists live (introspection quirk on an unrelated table). Never pipe blind answers or accept truncation; when push is blocked by an unrelated prompt, create the new table via manual SQL using drizzle-convention constraint names and verify with `\d`.

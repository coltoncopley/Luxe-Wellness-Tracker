...
**Server-side precondition rule for points-awarding completions:** Any "complete/finish" endpoint that awards points must enforce its preconditions server-side (e.g. ≥1 logged set before completing a workout), not just disable the button in the UI — review flags UI-only gating because a direct API call can farm points and streak days. Check the precondition with an owned-row query before mutating.

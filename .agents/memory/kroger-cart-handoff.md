...
- **Key presence ≠ key validity — probe before showing the feature.** The Kroger portal issues per-environment credentials; certification keys pass a "keys exist" check but api.kroger.com rejects them ("invalid credentials"), shipping a broken Connect button. Gate `status.enabled` on a cached client_credentials probe of the production token endpoint (401 = definitive → hide + retry ≤10min; 5xx/network = transient → stay enabled).
  **How to diagnose wrong-environment keys:** token endpoint works on `api-ce.kroger.com` but not `api.kroger.com`; the authorize page probe says "Invalid client_id" on prod when the app only exists in certification (vs a login page when only the secret is wrong).

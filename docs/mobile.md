...
- Home has a 12-tile "Explore" grid → stack screens under `app/explore/` (Skin Scan, Product Scan, Progress Photos, My Journey, Weekly Report, Meal Plan, Workouts, Beauty Passport, Dining Out Guide, Friends, Community, Hormone Health/BHRT). Home also shows the Wellness Streak card (milestone progress); Community screen includes the monthly challenges section (join + private progress bar).

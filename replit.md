...
- **Web pages:** Home (Wellness Score, AI briefing, offers, doctor tips) • Book • Weight • Food (meal logs, AI meal-photo scanner, quick-add search, Spoonacular chain search, Open Food Facts barcode scanner) • Restaurants/Dining Out Guide (18 curated menus + patient-added AI-grounded menus, DoorDash deep-links) • Glow • Mind (988 disclaimer) • Activity & Sleep (Oura sync, phone steps) • Luxe AI chat • Rewards (missions, tiers, referrals) • Progress Photos • Skin Scan • Product Scan • Beauty Passport • Friends (opt-in sharing) • Community (anonymous) • Notification settings + birthday • BHRT • Staff Portal `/staff` • Legal `/privacy` `/terms` `/support`. Privacy acknowledgment dialog forced on first sign-in (Me schema requires `privacyAcknowledged`).
